...
impl SMDHIcon {
    ...
    pub fn from_cia_tmd<T: Read + Seek>(f: &mut T, content_offset: u64) -> Result<Option<Self>, ParsingError> {
...
impl CIATitleMetadata {
    pub fn from_file<T: Read + Seek>(f: &mut T) -> Result<Self, ParsingError> {
        ...
        let signature_full_size: i64 = (signature_type.size() + signature_type.padding_size()).try_into().unwrap();
        let header_position = f.seek(SeekFrom::Current(signature_full_size))?;

        const TITLE_METADATA_HEADER_CONTENT_COUNT_OFFSET: i64 = 0x9E;
        f.seek(SeekFrom::Current(TITLE_METADATA_HEADER_CONTENT_COUNT_OFFSET))?;
        ...
        f.seek(SeekFrom::Start(header_position + CONTENT_CHUNK_RECORDS_OFFSET))?;
        ...
        for _ in 0..content_count {
            ...
            let content_chunk_record = CIAContentChunkRecord::from_bytes(&content_chunk_record.try_into().unwrap())?;
...
impl CIAContentChunkRecord {
    pub fn from_bytes(content_chunk_record_bytes: &[u8; 0x30]) -> Result<Self, ParsingError> {
        ...
        let content_index = u16::from_be_bytes(content_chunk_record_bytes[0x4..0x4+2].try_into().unwrap());
        let content_type = u16::from_be_bytes(content_chunk_record_bytes[0x6..0x6+2].try_into().unwrap());
        let content_size = u64::from_be_bytes(content_chunk_record_bytes[0x8..0x8+8].try_into().unwrap());
